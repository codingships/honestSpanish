...
El alta de personal separa identidad de autorización. La invitación se envía desde una ruta server-only de Supabase Auth y solo conserva `full_name` como metadato; nunca concede un rol mediante `user_metadata`. La petición queda auditada antes del efecto externo y vinculada mediante una huella de la identidad, sin guardar email o nombre en ese snapshot. Tras verificar el email y completar la atestación de adulto, una segunda acción explícita activa al profesor o promueve al administrador. Ambas transiciones exigen capacidad, bloquean perfiles con actividad de alumno, son idempotentes y escriben su auditoría en la misma transacción que el cambio de rol.
...
`packages` conserva la identidad estable y la proyección publicada de cada oferta. `package_catalog_drafts` guarda propuestas editables server-only; crear, editar, previsualizar o descartar un borrador no toca Stripe. Publicar verifica la cuenta y el modo de Stripe, crea o recupera de forma idempotente un Product y dos Prices —pago inicial y recurrencia— y después ejecuta una única RPC transaccional que incrementa la versión, retira el precio anterior, activa un `package_prices` inmutable, registra `checkout_v2_price_snapshots` y audita la mutación. Retirar oculta primero la oferta en base de datos y archiva después los recursos de Stripe; un fallo de limpieza externa no vuelve a abrir el checkout.

El snapshot de Checkout V2 conserva importe, moneda, periodo, número y duración de clases, además de la asignación exacta de céntimos por clase. La interfaz puede preparar ofertas distintas, pero solo permite listar públicamente el contrato exacto que el checkout actual sabe ejecutar; el resto se identifica expresamente como `Checkout pendiente` hasta generalizar compra, agenda y ciclo académico.
...
La garantía de Checkout V2 es una saga financiera duradera, no una suma calculada por el navegador ni una devolución aislada en Stripe. PostgreSQL decide la elegibilidad bajo bloqueo y congela en `checkout_v2_guarantee_operations` la compra, el ciclo pagado, el PaymentIntent y el reparto contractual por sesión; `checkout_v2_guarantee_operation_sessions` conserva cada sesión y sus céntimos como snapshot inmutable. Solo puede existir una operación por suscripción.

El orden irreversible es: validar el contrato local y remoto, cancelar inmediatamente la suscripción de Stripe, reflejar la terminación local e invalidar únicamente las sesiones no consumidas del ciclo, y crear la devolución parcial exacta con una clave de idempotencia estable. Los webhooks de refund y `charge.refunded` reconcilian la misma operación; nunca crean una segunda. Los estados pendientes quedan visibles y los resultados ambiguos pasan a revisión manual con ticket de soporte.

Una excepción de soporte a una cancelación tardía o no-show de cualquier sesión materializada se registra por separado en `checkout_v2_session_incident_resolutions`. Ese ledger es inmutable y auditado: reclasifica la incidencia para evaluar la garantía proporcional, pero no reabre ni reprograma una clase.

Tras confirmar Stripe la devolución, un único trabajo `guarantee_refund` envía la confirmación transaccional y registra el efecto en CRM. Las cancelaciones de Calendar de las sesiones no consumidas son trabajos deduplicados independientes; un fallo de Google o correo no altera el resultado financiero ya confirmado.

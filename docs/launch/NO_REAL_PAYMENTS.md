...
- Variables reales desplegadas en Cloudflare Pages, salvo que se ejecute con `--deployed-url`.
- Stripe live.
- Webhook live.
...
- `400` con `priceId is required`: el endpoint desplegado esta procesando checkout como habilitado o no tiene el fail-closed activo; corregir `CHECKOUT_ENABLED=false` en Cloudflare Pages para ese entorno solo basta si el despliegue ya contiene el guard que lee esa variable. Si `local_deployment_gap` avisa de cambios solo locales, empaquetar/commitear y redeployar primero el codigo actual.
...
Si se va a usar un build local como paquete de despliegue, ejecutar primero `corepack pnpm build`, repetir `corepack pnpm launch:staging-no-real-payments-remediation` y revisar `pages-staging-build-manifest.json`. El manifest debe mostrar `readyForStagingDeployPackage=true`: registra rutas y hashes del build que contienen el guard, valida `dist/server/wrangler.json`, `entry.mjs`, assets, `CHECKOUT_ENABLED=false`, `nodejs_compat` y limites basicos de Pages, sin guardar el contenido compilado. Si falta `dist`, si el manifest no encuentra `Checkout is disabled` y `CHECKOUT_ENABLED`, o si falla la estructura basica de Pages, no usar ese build para staging.
...
Usar el `approval-request.md` antes de tocar Cloudflare: limita el scope al proyecto Pages staging, exige preflight de cuenta/proyecto/entorno, exige revisar el `pages-staging-build-manifest.json` si se despliega un build local, permite solo `CHECKOUT_ENABLED=false` cuando el guard ya esta desplegado o redeploy de config/codigo actual cuando falta el guard, y deja fuera production Pages, Stripe live, pagos reales, secretos finales y dominio/Search Console.

Usar `manual-evidence-dry-run.txt` solo despues de que el post-fix probe desplegado pase. Primero ejecutar `corepack pnpm launch:no-real-payments -- --deployed-url https://espanol-honesto-staging.pages.dev`; si devuelve 403 `Checkout is disabled`, sustituir la nota generica por evidencia concreta y anadir `--write`. Si sigue devolviendo `400 priceId is required`, no registrar `payments_staging` como cerrado.

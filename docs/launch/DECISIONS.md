...
Decision de URLs Auth staging: `site_url` debe ser el origen canónico `https://staging.espanolhonesto.com` y `uri_allow_list` debe contener rutas exactas, sin `*`, `**`, clases, llaves, escapes ni equivalentes codificados. Se permiten únicamente las tres confirmaciones localizadas `/api/auth/confirm?lang=es|en|ru` y las tres recuperaciones `/{es|en|ru}/reset-password`, además de entradas exactas ya existentes. El runner bloquea antes de cualquier PATCH si el baseline o la lista requerida contiene un comodín amplio; no lo conserva ni lo elimina sin una autorización distinta.

Decisión de hostname staging: usar `https://staging.espanolhonesto.com` como origen público canónico. No renombrar el subdominio global de cuenta `alindev95.workers.dev`, porque afectaría a otros Workers. Las Preview URLs del Worker web quedaron desactivadas en el deploy del 2026-07-13; mantener temporalmente la URL directa `workers.dev` solo para rollback y para el webhook Stripe test existente, y retirarla únicamente después de verificar Stripe test, Turnstile, correos y smokes sobre el dominio custom. La predictibilidad del hostname no se considera un control de acceso; la seguridad depende de Auth/RBAC/RLS, firmas de webhook, gates de entorno, checkout desactivado y noindex global de staging.

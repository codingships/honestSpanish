...
Decisión de hostname staging: usar `https://staging.espanolhonesto.com` como origen público canónico. No renombrar el subdominio global de cuenta `alindev95.workers.dev`, porque afectaría a otros Workers. Mantener temporalmente la URL directa del Worker web solo para rollback y para el webhook Stripe test existente; desactivar Preview URLs en el siguiente deploy y retirar `workers.dev` únicamente después de verificar el dominio custom, Supabase Auth, Stripe test, Turnstile, correos y smokes. La predictibilidad del hostname no se considera un control de acceso; la seguridad depende de Auth/RBAC/RLS, firmas de webhook, gates de entorno, checkout desactivado y noindex global de staging.

...
7. Aplicar y verificar primero en staging, y en este orden, `20260712112000`, `20260712114000` y `20260712114500` con `pnpm launch:supabase-staging-hardening`. El post-check debe cerrar el contrato del modelo (`leads.updated_at`, enum/defaults, grants, ausencia de `public.is_admin`, trigger e indices), el solape de disponibilidad y la politica 18+. Production solo acepta un summary real `APPLIED_AND_VERIFIED`, no un plan ni una afirmacion de que ya estaba aplicado. Cerrar tambien `pnpm launch:sentry-production-hardening` con ejecucion aprobada: el summary debe ser fresco, apuntar a `honestspanish/espanol-honesto-astro` production y quedar `HARDENED_AND_VERIFIED` con scrubbing IP y los dos workflows exactos.

...
4. Ejecutar primero el preview de `pnpm launch:supabase-production-fixture-cleanup -- preview`. La ejecucion aprobada exige ademas `--auth-inert-evidence <auth-inert-receipt.json>`, vuelve a comprobar el snapshot v2 y, en una unica transaccion, elimina exactamente 111 filas de la tabla legacy `public.jobs`, la elimina sin `CASCADE`, elimina los 2 `support_tickets` y el resto de fixtures publicos, conserva solo `group`, `standard`, `hybrid` y `bootcamp`, y limpia sus referencias Stripe locales. Debe emitir `public-cleanup-receipt.json` vinculado al SHA-256 Auth; Auth, Storage, Stripe y Google quedan fuera de esta transaccion.

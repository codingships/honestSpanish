...
- [x] Preflight, backup, limpieza v2 y rollout production preparados fail-closed: `pnpm launch:supabase-production-readonly-preflight` y `pnpm launch:supabase-production-rollout-plan` fijan el target exacto `vkkahxsybhbutszerawz`, 22 migraciones por hash en seis olas, `20260710150000` excluida, verificacion por efectos e historial source-bound, y prohibicion de `supabase db push`, `supabase migration repair`, restore o switch automaticos. Los modos por defecto son locales/read-only y checkout debe permanecer desactivado. Evidencia base: `outputs/launch-supabase-production-readonly-preflight/2026-07-12T10-03-23-365Z/summary.md`, `outputs/launch-supabase-production-rollout-plan/2026-07-12T10-05-21-802Z/summary.md`.
- [x] Runner Auth production preparado y probado sin ejecutar production: plan por defecto sin red/writes; preflight agregado sin emails/UUID; borrado secuencial reanudable de 136 usuarios; preservacion exacta de admin/profesor; rotacion aleatoria no retenida; cero refresh sessions; cuarentena por JWT; finalize bloqueado hasta receipt de las 22 olas y expiracion; cuatro aprobaciones separadas; cero emails/Storage/Stripe/Google. Evidencia: `scripts/launch/supabase-production-auth-cleanup.ts`, `scripts/launch/supabase-production-auth-cleanup-shared.ts`, `tests/unit/supabase-production-auth-cleanup.test.ts`, `outputs/launch-supabase-production-auth-cleanup/2026-07-12T11-46-46-222Z/summary.json`.

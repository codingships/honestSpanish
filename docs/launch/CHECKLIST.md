...
- [ ] Ejecutar bajo aprobaciones separadas y en orden: `pnpm launch:cloudflare-production-queues`, `pnpm launch:cloudflare-production-fulfillment-bootstrap`, `pnpm launch:cloudflare-production-fulfillment-bootstrap-secrets`, `pnpm launch:cloudflare-production-worker-phase1`, `pnpm launch:cloudflare-production-worker-bootstrap-secrets`; dejar `pnpm launch:cloudflare-production-worker-secrets`, `pnpm launch:cloudflare-production-fulfillment-secrets` y `pnpm launch:cloudflare-production-fulfillment-enable` para la ventana final. Las Queues se crean primero sin consumidor activo; cada runner valida las fases previas; los bootstrap atestiguan providers ausentes y las fases activas atestiguan identidad, versión, modo operativo y Supabase. Evidencia canónica: `docs/launch/CLOUDFLARE_PRODUCTION.md`.
...
- [ ] `FULFILLMENT_WORKER_URL` production configurado en el GitHub environment `Production` durante el cierre final. Staging CI usa directamente la URL pública canónica `https://espanol-honesto-fulfillment-staging.alindev95.workers.dev` y ya no depende de un secret para ese dato no sensible.
...
- [x] Baseline billing de `database_readiness` reconciliado en staging. Evidencia 2026-07-10: las migraciones `20260710205031`, `20260710215712`, `20260710221846` y `20260710223900` se validaron primero en transaccion con `ROLLBACK`, cada dry-run propuso solo la siguiente migracion, se aplicaron a `mzjyvmlxfpzdfdjzxxyj` y `supabase db lint --schema public --level error` termino sin hallazgos. Las cuatro migraciones RC posteriores ya se aplicaron; el cierre vigente queda pendiente solo de `20260712195500` en la fila 207. Production no se ha tocado.
...
- [x] Preflight y plan production fail-closed regenerados tras el contrato final de sesiones: `pnpm launch:supabase-production-readonly-preflight` confirmó por lectura DB el target exacto `vkkahxsybhbutszerawz`, 25 migraciones semánticas pendientes, `20260710150000` excluida y cero ambigüedades; `pnpm launch:supabase-production-rollout-plan` cerró `PLAN_ONLY_READY` con los 25 hashes en siete olas y cero writes. `supabase db push` y `supabase migration repair` siguen prohibidos. Evidencia fresca: `outputs/launch-supabase-production-readonly-preflight/2026-07-12T19-55-38-770Z/summary.md` y `outputs/launch-supabase-production-rollout-plan/2026-07-12T19-55-43-679Z/summary.md`.
- [x] Runner Auth production preparado y probado sin ejecutar production: plan por defecto sin red/writes; preflight agregado sin emails/UUID; borrado secuencial reanudable de 136 usuarios; preservacion exacta de admin/profesor; rotacion aleatoria no retenida; cero refresh sessions; cuarentena por JWT; finalize bloqueado hasta receipt de las 25 migraciones en siete olas y expiracion; cuatro aprobaciones separadas; cero emails/Storage/Stripe/Google. Evidencia local fresca con contador 25: `outputs/launch-supabase-production-auth-cleanup/2026-07-12T19-57-03-415Z/summary.json` (`PLAN_ONLY_READY`, sin red ni writes).
...
- [ ] Aplicar y verificar en Supabase staging la cola `20260712195500_harden_sessions_status_contract.sql`; el preflight remoto reconoce `20260712112000`, `20260712114000`, `20260712114500` y `20260712115000` como prefijo exacto ya aplicado —version, nombre y SQL histórico no vacío— y el runner solo puede escribir la quinta. Verificar `sessions.status` `NOT NULL`, default/check, cero valores inválidos y conservar todos los contratos RC anteriores. Evidencia read-only: `outputs/launch-supabase-staging-hardening-runner/2026-07-12T20-12-39-802Z/summary.md` (`PREFLIGHT_VERIFIED`, `historyState=valid_prefix`, cero writes).
...
- [ ] Aplicar production con `pnpm launch:supabase-production-rollout` y todos sus receipts: `processed_at_small_fix` (1), `base_model_reconciliation` (1), `application_schema` (7), `runtime_and_policy` (7), `billing_contract` (4), `fulfillment_ledger` (1) y `deferred_rc_hardening` (4). Verificar cada ola y el recibo final manteniendo checkout desactivado. No usar el runner legacy `launch:supabase-processed-at-cleanup-runner`: staging ya esta cerrado y ese runner permanece fail-closed para escrituras.
- [ ] Tras las 25 migraciones en siete olas y el vencimiento de la cuarentena, finalizar Auth: crear solo los perfiles/private de admin y profesor y obtener `auth-policy-receipt.json`. No abrir trafico antes.
...
- [x] Checkout test y ciclo billing staging cerrados sin abrir cobros públicos: Sandbox dedicado `espanolhonesto-staging` España/EUR, 4 Products/12 Prices, Portal, webhook exacto, compra completada y conciliada, renovación/fallo/recuperación/cancelación y reembolsos parcial+total verificados. El gate permaneció desactivado fuera del ensayo; Stripe Live sigue final-only. Evidencia: `outputs/launch-staging-billing-lifecycle/2026-07-12T08-15-09-474Z/summary.json`, `outputs/launch-staging-smoke-rehearsal-runner/2026-07-12T08-49-24-788Z/summary.md` y filas 252-254.
...
- [ ] Repetir advisors de seguridad/rendimiento después de aplicar `20260712195500` en staging. Tras las cuatro migraciones RC anteriores el conector devolvió 8 avisos de seguridad y 43 de rendimiento, sin críticos y sin los 13 `auth_rls_initplan`; falta la lectura posterior al último contrato de sesiones.
- [ ] Flujo registro antes de pago y recuperación de contraseña validados de extremo a extremo en staging. Parcial: login real de admin/profesor y bloqueo 18+ del alumno validados el 2026-07-10; el runner local de `uri_allow_list` ya fija las seis rutas exactas ES/EN/RU de confirmación y `reset-password` y rechaza comodines amplios, pero sigue plan-only. Falta inspeccionar/aplicar el allowlist hospedado con aprobación separada, completar un alta nueva deliberada, confirmar el email, recorrer recuperación y ejecutar cleanup sin tocar las tres cuentas operativas.

## Operacion

Las filas de simulacro de incidente, alertas Sentry y rollback son bloqueadores del Release Candidate, no tareas final-only. `pnpm launch:status`, `pnpm launch:rc` y `pnpm launch:secondary-review` las leen directamente de esta seccion y fallan mientras sigan abiertas o desaparezcan sin evidencia.

...
- [x] Landing publica incluye que incluye el curso, a quien va dirigido, comunidad con encaje sin grupos garantizados e Irene como profesora con placeholder de imagen. Evidencia: `src/components/LandingPage.astro`.
...
- [ ] Cloudflare Fulfillment Worker con `FULFILLMENT_WORKER_URL`, `PUBLIC_SITE_URL`, `INTERNAL_JOB_SECRET` y `CRON_SECRET`. Staging verificado, Pages staging conectado al Worker, smoke seguro de job completado, reminders dry run 200, Google Calendar read-only 200, Resend staging HTTP 200 y secretos staging listados por nombre; falta production Worker y smoke con Drive real, ambos final-only antes de produccion.

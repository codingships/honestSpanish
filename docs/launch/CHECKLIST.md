...
- [ ] Bootstrap del primer Checkout test: aprobar exactamente `STAGING_CHECKOUT_BOOTSTRAP_APPROVAL` y `STAGING_CHECKOUT_GATE_APPROVAL`, ejecutar `pnpm launch:staging-smoke-rehearsal-runner -- --bootstrap-checkout-approved`, comprobar que prepara una unica sesion Standard/1 mes bajo Test Clock, que la URL solo llega al portapapeles local y que el resumen verifica rollback `false` antes de completar la compra.

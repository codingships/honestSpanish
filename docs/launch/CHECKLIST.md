...
- [x] Preflight, backup, limpieza v2 y rollout production preparados fail-closed: `pnpm launch:supabase-production-readonly-preflight`, `pnpm launch:supabase-production-connector-preflight` y `pnpm launch:supabase-production-rollout-plan` fijan el target exacto `vkkahxsybhbutszerawz`, 23 migraciones por hash en siete olas, `20260710150000` excluida, coherencia cruzada de agregados, verificacion por efectos e historial source-bound, y prohibicion de `supabase db push`, `supabase migration repair`, restore o switch automaticos. Los modos por defecto son locales/read-only y checkout debe permanecer desactivado. Los hashes de `20260712112000` y `20260712114000` quedan fijados por los runners; cualquier cambio de SQL invalida la evidencia. Evidencia actual: `outputs/launch-supabase-production-connector-preflight/2026-07-12T15-43-56-403Z/summary.json`, `outputs/launch-supabase-production-rollout-plan/2026-07-12T15-44-50-587Z/summary.md`.
- [x] Runner Auth production preparado y probado sin ejecutar production: plan por defecto sin red/writes; preflight agregado sin emails/UUID; borrado secuencial reanudable de 136 usuarios; preservacion exacta de admin/profesor; rotacion aleatoria no retenida; cero refresh sessions; cuarentena por JWT; finalize bloqueado hasta receipt de las 23 migraciones en siete olas y expiracion; cuatro aprobaciones separadas; cero emails/Storage/Stripe/Google. Evidencia: `scripts/launch/supabase-production-auth-cleanup.ts`, `scripts/launch/supabase-production-auth-cleanup-shared.ts`, `tests/unit/supabase-production-auth-cleanup.test.ts`, `outputs/launch-supabase-production-auth-cleanup/2026-07-12T11-46-46-222Z/summary.json`.
...
- [ ] Aplicar realmente y verificar en Supabase staging, en orden, `20260712112000`, `20260712114000` y `20260712114500`; production exige cierre `APPLIED_AND_VERIFIED`, no plan ni `ALREADY_APPLIED`. Verificar `leads` obligatorio/defaults, grants/helper admin, `reminder_sent`, policy alumno -> profesor, indices, duraciones 30/40/50, trigger/solapes y signup 18+.
...
- [ ] Aplicar production con `pnpm launch:supabase-production-rollout` y todos sus receipts: `processed_at_small_fix` (1), `base_model_reconciliation` (1), `application_schema` (7), `runtime_and_policy` (7), `billing_contract` (4), `fulfillment_ledger` (1) y `deferred_rc_hardening` (2). Verificar cada ola y el recibo final manteniendo checkout desactivado. No usar el runner legacy `launch:supabase-processed-at-cleanup-runner`: staging ya esta cerrado y ese runner permanece fail-closed para escrituras.
- [ ] Tras las 23 migraciones en siete olas y el vencimiento de la cuarentena, finalizar Auth: crear solo los perfiles/private de admin y profesor y obtener `auth-policy-receipt.json`. No abrir trafico antes.

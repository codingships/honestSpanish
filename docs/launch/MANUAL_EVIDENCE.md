...
- `outputs/launch-final-readiness/<timestamp>/final-smoke-worksheet.md`: hoja generada por `pnpm launch:final-readiness`; separa el arnes tecnico integral, exclusivo de staging, del smoke minimo/manual de production.

El ciclo integral (Checkout real test, webhook, Drive, email, reserva, Doc, Calendar/Meet, recordatorio, cancelacion y retry) se demuestra en staging. No ejecutar el arnes staging en production ni fabricar eventos Stripe.

- `outputs/launch-staging-billing-lifecycle/<timestamp>/summary.json` y `summary.md`: evidencia canonica generada por `pnpm launch:staging-billing-lifecycle`; debe acreditar la Sandbox/Supabase/sesion exactas, mutacion autorizada, checkpoint `complete`, eventos y estados terminales, revalidacion final y pago inicial preservado. `pnpm launch:staging-billing-lifecycle:preflight` es read-only; `:resume` solo se usa tras una interrupcion con `outputs/launch-staging-billing-lifecycle/checkpoints/<session>.json` valido, que no se edita ni se borra.
...
- `outputs/launch-final-smoke-execution-pack/<timestamp>/approval-request-final-smoke.md` y `production-minimal-smoke-checklist.md`: aprobacion y checklist para el smoke minimo/manual de production. `scripts/smoke/real-env-smoke.ts` no se ejecuta nunca contra production.
- `outputs/launch-final-smoke-execution-pack/<timestamp>/approval-request-staging-smoke.md` y `staging-preflight-checklist.md`: el smoke exige `CHECKOUT_ENABLED_OVERRIDE=false` durante toda la ejecucion y no autoriza escrituras Cloudflare.
- `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/summary.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/staging-smoke-command-manifest.json`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/staging-smoke-execution-plan.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/approval-gate.md` y `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/rollback-after-staging-smoke.md`: `--execute-approved` ejecuta primero `real-env-smoke.ts --preflight-only --expect-checkout-override false` y no inicia ningun write si fallan catalogo, cuentas, allowlist, Checkout completado, la ruta explicita `SMOKE_BILLING_LIFECYCLE_EVIDENCE_PATH`, su validacion/revalidacion viva o el probe 403. Reutiliza solo las tres cuentas `TEST_*`, crea cero usuarios Auth y no necesita leer el buzon del alumno.

Generar primero el paquete plan-only con `pnpm launch:staging-smoke-rehearsal-runner`; usar `--execute-approved` solo tras la aprobacion del smoke, el ciclo billing canonico y el preflight completo. El bootstrap estrecho no es un paso de launch: queda solo como excepcion manual fuera del runner, con ownership externo explicito del gate y verificacion posterior en `false`.
...
| `payments_staging` | Compra Stripe test staging realmente completada, webhook delivery real, subscription/payment, portal y reconciliacion verificados antes de activar pagos reales; no se aceptan eventos sinteticos como sustituto. Si se lanza sin pagos, checkout queda desactivado/oculto/bloqueado hasta cierre final. |
...
| `integration_readiness` | Stripe test ensayado y Stripe live preparado para pagos reales desde el primer dia, con `CHECKOUT_ENABLED_OVERRIDE=false` hasta Go/No-Go y rollback probado; paquete `pnpm launch:integration-final-package`, Stripe webhook test-mode con `pnpm launch:stripe-webhook-cutover-runner`, Turnstile dominios con `pnpm launch:turnstile-domain-closure-runner`, Sentry issue triage con `pnpm launch:sentry-issue-triage-runner`, Cloudflare Pages-vs-Worker/domain ownership, snapshot `pnpm launch:cloudflare-production-runtime-readonly`, preflight `pnpm launch:cloudflare-production-runtime-cutover-preflight`, `cloudflare-production-worker-variable-matrix.md`, production Worker secret-name posture, cutover Cloudflare generado por `pnpm launch:cloudflare-production-runtime-cutover`, runner gated `pnpm launch:cloudflare-production-worker-phase1`, runner gated `pnpm launch:cloudflare-production-worker-secrets`, Supabase processed_at mediante la ola source-bound `processed_at_small_fix` de `pnpm launch:supabase-production-rollout`, Google, Resend, Sentry y fulfillment/reminder worker revisados/configurados. |
...
| `final_smoke` | El ciclo billing real se demuestra primero con `outputs/launch-staging-billing-lifecycle/<timestamp>/summary.json`; el smoke integral (Checkout/webhook ya reconciliados, Drive, email, reserva, Doc, Calendar/Meet, recordatorio, cancelacion y retry) se demuestra despues con `outputs/real-env-smoke/<timestamp>/summary.md`. Production se cierra con `production-minimal-smoke-checklist.md`, de forma manual y minima. No ejecutar los arneses staging en production ni fabricar eventos Stripe. |

...
- `outputs/launch-final-smoke-execution-pack/<timestamp>/approval-request-staging-smoke.md`, `approval-request-staging-checkout-gate.md` y `staging-preflight-checklist.md`: el smoke y el cambio temporal de `CHECKOUT_ENABLED_OVERRIDE` tienen aprobaciones distintas. La segunda nombra solo Worker `espanolhonesto-staging` y permite al runner aprobado ser dueño del intervalo `false → true → false`; el rollback ocurre en `finally` y solo se acepta con atestacion firmada y probe 403.
- `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/summary.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/staging-smoke-command-manifest.json`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/staging-smoke-execution-plan.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/approval-gate.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/cloudflare-checkout-gate-approval.md` y `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/rollback-after-staging-smoke.md`: el modo `--bootstrap-checkout-approved` permite preparar el primer Checkout de una Sandbox nueva sin fingir evidencia previa, bajo `STAGING_CHECKOUT_BOOTSTRAP_APPROVAL` y con URL solo en portapapeles; el modo `--execute-approved` ejecuta primero `real-env-smoke.ts --preflight-only` y no inicia ningun write si fallan catalogo, cuentas, allowlist, Checkout completado, ciclo manual o gate desplegado. Ambos restauran/verifican el gate en `false`. Reutilizan solo las tres cuentas `TEST_*`, crean cero usuarios Auth y no necesitan leer el buzon del alumno.

Generar primero el paquete plan-only con `pnpm launch:staging-smoke-rehearsal-runner`; en una Sandbox vacia usar `--bootstrap-checkout-approved` solo tras las aprobaciones bootstrap+Cloudflare, completar y revisar el ciclo real, y usar `--execute-approved` solo despues de las aprobaciones smoke+Cloudflare y del preflight completo.
...
| `final_smoke` | El ciclo integral (Checkout real test, webhook, Drive, email, reserva, Doc, Calendar/Meet, recordatorio, cancelacion y retry) se demuestra en staging con `outputs/real-env-smoke/<timestamp>/summary.md`; production se cierra despues con `production-minimal-smoke-checklist.md`, de forma manual y minima. No ejecutar el arnes staging en production ni fabricar eventos Stripe. |

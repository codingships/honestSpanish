...
- En una Sandbox nueva, el primer Checkout real se prepara con el mismo runner y `--bootstrap-checkout-approved`, tras la aprobacion exacta adicional `STAGING_CHECKOUT_BOOTSTRAP_APPROVAL`. Este modo no exige aun evidencia de pago completado: valida Supabase/Stripe/catalogo/alumno, abre el gate solo para crear un Customer test bajo Test Clock y una sesion Standard de un mes, copia la URL verificada solo al portapapeles local y restaura/verifica `false` en `finally`. La sesion se completa despues con el gate ya cerrado; su URL no se guarda en outputs, logs ni evidencia. Si el child falla, el cleanup de artefactos incompletos empieza solo despues de verificar el rollback; con gate ambiguo se omite y se prioriza el cierre manual inmediato.

...
## Mapa Operativo Actual (2026-07-17)

| Capa | Staging confirmado | Production confirmado | Estado/accion |
| --- | --- | --- | --- |
| Web Cloudflare objetivo | Worker `espanolhonesto-staging`; origen canónico activo `https://staging.espanolhonesto.com`; URL directa transitoria `https://espanolhonesto-staging.alindev95.workers.dev` | Worker `espanolhonesto`, versión vigente HMAC-only `8087b6dc-ff94-4af0-8de0-2a923e58e99f` al 100 % en `WEB_RUNTIME_MODE=bootstrap` | Staging tiene DNS, TLS y routing verificados. Production está creada pero inerte: checkout/email a cero, proveedores ausentes y sin dominios finales; Pages conserva todavía los dominios públicos hasta la ventana de cutover. |
...
| Fulfillment | Worker `espanol-honesto-fulfillment-staging`, conectado al web por `FULFILLMENT_SERVICE` | Worker `espanol-honesto-fulfillment-production`, versión `c4854b76-a245-4afd-b42c-8ce6a8b5a36c` al 100 % | Staging tiene cron/secrets y pasó el smoke integral. Production está en bootstrap inerte con un único HMAC compartido con web, cero Cron, proveedores ausentes, health 200 y operación 503. Queue/DLQ existen activas, sin backlog ni productores/consumidores mientras siga inerte. |
| Supabase | Proyecto `mzjyvmlxfpzdfdjzxxyj`, 48 migraciones hasta `20260713161300` | Proyecto `vkkahxsybhbutszerawz`, 49 entradas históricas | Proyectos separados y sanos. Production tiene aplicadas las 25 migraciones RC, excluye las dos staging-only y conserva exactamente admin + profesor, dos perfiles/private mínimos, cuatro paquetes, cinco franjas L–V 09:00–18:00 y cero filas transaccionales/CRM/billing/fulfillment. Signup sigue desactivado; solo falta el backup lógico fresco post-cierre y la reatestación read-only ligada al SHA canónico. |
...
| Resend | Staging desplegado y verificado en `allowlist`, con presupuesto persistente de 10 destinatarios/dia y 100/mes | Pendiente de verificacion production | Pasarela unica fail-closed y presupuesto persistente activos en staging; production conserva el limite diseñado de 80/dia y 2.400/mes, pendiente de cierre final. |
| Sentry | `.env.staging` local no captura; CI/deploy necesita DSN y `SENTRY_ORG`/`SENTRY_PROJECT` | Scrubbing IP y dos workflows `production` exactos activos desde 2026-07-15; quedan triage/riesgo de incidencias y rotación final | Sin org/project no se suben sourcemaps aunque exista token. |
...
- `STRIPE_EXPECTED_WEBHOOK_HOSTS` opcional para sobrescribir el host esperado. El cutover test a `staging.espanolhonesto.com` quedó aplicado y verificado el 2026-07-15; production exigirá el canónico `espanolhonesto.com` y no se configura un webhook duplicado en `www`. `workers.dev` ya no es una dependencia del webhook test y su retirada queda sujeta al smoke posterior al deploy y a una autorización Cloudflare separada.
...
El Fulfillment Worker activo declara un cron horario (`0 * * * *`) para reconciliar jobs pendientes y recordatorios. En staging, `FULFILLMENT_QUEUE` publica señales `process_due` y el mismo Worker las consume con batch/concurrencia uno, cinco reintentos y DLQ; así Google/Resend no dependen de los 30 segundos posteriores a una respuesta HTTP. Supabase `fulfillment_jobs` conserva el estado durable. Staging se despliega activo antes del Astro Worker. Production se crea primero con el entorno `production_bootstrap`: `FULFILLMENT_RUNTIME_MODE=bootstrap`, email desactivado, cuotas cero, `crons=[]` y rutas operativas bloqueadas con 503. Después se despliega el bootstrap del Astro Worker que declara `FULFILLMENT_SERVICE`; los secrets completos se cargan mientras fulfillment sigue inerte, `launch:cloudflare-production-worker-secrets` hace el build/deploy web activo con checkout falso y compensación automática al bootstrap, y solo otro gate separado despliega fulfillment `--env production`. Ese enable exige un preflight temprano y vuelve a leer Queue/DLQ y Stripe Live en el mismo proceso inmediatamente antes del write. Su aprobación exacta solo se acepta desde el entorno inicial, nunca desde el fichero dotenv, y el intento queda protegido por el lock local `outputs/launch-cloudflare-production-fulfillment-enable/execution.lock` y el checkpoint CAS revisionado del mismo directorio. La Queue production no se crea ni se vincula durante staging. No usar `fetch()` público entre Workers de la misma cuenta.
...
- `pnpm launch:supabase-production-history-reconciliation -- --capture-readonly` es la captura canonica de la excepcion historica: exige que las dos URLs locales identifiquen exactamente production, ejecuta mediante `psql` solo el SQL allowlisted por SHA-256 con read-only en sesion y transaccion, mantiene el snapshot bruto solo en memoria y emite un manifiesto agregado que caduca a los 15 minutos.
...
- `pnpm launch:supabase-production-rollout-plan` es local/plan-only: no conecta a Supabase, no genera SQL de limpieza ni bundles de aplicacion, separa autorizaciones y produce manifiesto, una plantilla de backup que refleja el mismo contrato que el recibo real, preservacion, frases exactas, verificacion y rollback.
- `pnpm launch:supabase-production-logical-backup` crea, solo con aprobacion exacta, un dump custom `public` + `auth` fuera del repositorio en un directorio Windows EFS verificado. Su recibo v1 no contiene la ruta ni credenciales y acredita SHA-256, tamano, EFS y TOC `pg_restore --list`.
...
- `pnpm launch:supabase-production-auth-cleanup` separa reduccion/cuarentena y finalizacion: primero deja dos Auth sin perfiles; despues de las 25 migraciones en siete olas y del vencimiento de la cuarentena crea los dos perfiles minimos.
- `pnpm launch:production-availability` solo se habilita despues de un `auth-policy-receipt.json` cerrado: el preflight liga la huella del conjunto preservado a los UUID live, exige roles exactos `admin=1, teacher=1, student=0`, fija el profesor preservado y crea L-V 09:00-18:00 en `Europe/Madrid`. Tras cualquier intento de escritura, incluso timeout/error del cliente, ejecuta una lectura nueva y clasifica el resultado como aplicado, rollback verificado o ambiguo; un estado ambiguo nunca se presenta como ausencia de escritura ni permite reintento ciego.
- `pnpm launch:supabase-production-rollout` es plan-only por defecto. Su ejecucion final exige evidencia fresca y vinculada de preflight sin drift, backup, limpieza publica, Auth en cuarentena, politica Google, hardening staging y el recibo v2 rollout-eligible de Sentry production, mas aprobacion exacta y atestacion de checkout desactivado. Para cualquier write pendiente tambien exige `--backup-artifact <la-misma-ruta.dump>`: recalcula SHA-256, confirma Windows EFS y vuelve a leer el TOC justo antes del primer write, sin persistir la ruta.
- `pnpm launch:supabase-production-post-closure-backup` es el único runner del snapshot mínimo posterior al rollout. Exige `main` limpio, SHA canónico, `production-inert-final-receipt.json` fresco y destino EFS nuevo; hace únicamente GET Auth y SQL read-only, abre el archivo con exclusividad `wx`, vuelca `public + auth`, compara el TOC con el inventario live y emite un receipt sin ruta. No sustituye ni reabre el backup histórico del rollout.

El inventario de partida del 2026-07-13 encontró 48 migraciones locales. Staging registra y verifica las cuatro migraciones RC `20260712112000`-`20260712115000`, `20260712195500_harden_sessions_status_contract.sql` y la migración exclusiva de hostname staging `20260713161300_allow_staging_custom_hostname.sql`; no queda una cola local pendiente para staging. El 2026-07-17 production aplicó y verificó las 25 migraciones semánticas en siete olas y cerró con 49 entradas históricas. `20260710150000_staging_integration_smoke_runs.sql` y `20260713161300_allow_staging_custom_hostname.sql` siguen siendo exclusivamente staging y están ausentes de production. Evidencia: `outputs/launch-supabase-production-rollout-runner/2026-07-17T10-20-29-557Z/production-rollout-receipt.json`.

El preflight production del 2026-07-14 conserva ademas cuatro derivas historicas que bloquean fail-closed salvo excepcion exacta, fresca y revisada: la version `009` figura remotamente como `jobs` frente al source local `009_launch_catalog_and_fulfillment.sql`; `021_harden_session_write_policies` aparece en `021` y `20260703192245`; `022_track_stripe_webhook_processing_state` aparece en `022` y `20260703192307`; y `20260702124757_harden_profile_role_trigger` aparece tambien en `20260703192329`. Evidencia agregada: `outputs/launch-supabase-production-readonly-preflight/2026-07-14T18-01-14-004Z/summary.json` (`versionNameMismatchCount=1`, `duplicateSemanticHistoryCount=3`). Ningun consumidor acepta estos warnings por si solos: el manifiesto v2 source-bound debe demostrar hashes, bytes, aliases canonicos y 29 efectos exactos, y la ejecucion exige ademas opt-in y aprobacion exacta. No usar `migration repair`.

Los tipos del conector de staging coinciden con `src/types/database.types.ts` en las 24 tablas después de aplicar `20260712195500`; `20260713161300` solo cambia una restricción de hostname y no altera tipos. Las diferencias RPC restantes son los overrides nulos deliberados que PostgreSQL/Supabase no infiere desde cuerpos PL/pgSQL.

No usar `supabase db push` contra este historial divergente ni `supabase migration repair` para igualarlo visualmente. Los aliases y el choque de nombre se verifican por efectos de esquema; cualquier `versionNameMismatch` o duplicado semantico bloquea al consumidor. La primera ola contiene solo `20260703211451_drop_processed_webhook_processed_at_default.sql`; la segunda es `base_model_reconciliation` y las otras cinco conservan su orden de dependencias. Toda ola con writes exige el recibo y el dump actual revalidado; las seis posteriores a `processed_at_small_fix` exigen ademas cierre de fixtures previo. El runner verifica cada ola y emite el recibo final solo tras las 25 migraciones. Las migraciones staging-only permanecen ausentes y checkout permanece desactivado durante todo el rollout.

El runner histórico `pnpm launch:supabase-processed-at-cleanup-runner` está retirado para escrituras y falla antes de red/SQL aunque se pase `--execute-approved`. Staging y production ya registran `20260703211451`; production lo cerró como primera ola del rollout del 2026-07-17 y verificó ambos defaults `NULL` con agregados webhook limpios. No repetir la ola ni usar el runner legacy para refrescar evidencia; una comprobación futura debe ser exclusivamente read-only.

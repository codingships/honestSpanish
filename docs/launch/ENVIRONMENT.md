...
El inventario read-only vigente de 2026-07-12 encuentra 47 migraciones locales. Staging ya registra y verifica las cuatro migraciones RC anteriores (`20260712112000`, `20260712114000`, `20260712114500`, `20260712115000`) y conserva exactamente una cola pendiente: `20260712195500_harden_sessions_status_contract.sql`, que hace `sessions.status` obligatorio y reconstruye su `CHECK` reproducible. El runner reconoce esas cuatro como prefijo exacto y no las reaplica. Production mantiene 25 migraciones semanticas pendientes; el preflight fresco las clasifica todas en siete olas, sin ambiguedades ni migraciones sin plan. `20260710150000_staging_integration_smoke_runs.sql` sigue siendo exclusivamente staging y nunca se aplica a production.

Los tipos del conector de staging coinciden con `src/types/database.types.ts` en las 24 tablas salvo la nulabilidad deliberadamente adelantada de `sessions.status`, ligada a `20260712195500`. Tras aplicar esa cola se deben regenerar los tipos y exigir cero deriva de tablas; las diferencias RPC restantes son los overrides nulos deliberados que PostgreSQL/Supabase no infiere desde cuerpos PL/pgSQL.

...
| --- | --- | --- | --- |
| Web Cloudflare objetivo | Worker `espanolhonesto-staging`; origen canónico activo `https://staging.espanolhonesto.com`; URL directa transitoria `https://espanolhonesto-staging.alindev95.workers.dev` | Worker objetivo `espanolhonesto` | Staging tiene DNS, TLS y routing verificados; Preview URLs web desactivadas y `workers.dev` conservado para rollback/webhook test. El Worker production aun no existe. |

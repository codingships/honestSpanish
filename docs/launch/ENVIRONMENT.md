...
## Mapa Operativo Actual (2026-07-16)

| Capa | Staging confirmado | Production confirmado | Estado/accion |
| --- | --- | --- | --- |
| Web Cloudflare objetivo | Worker `espanolhonesto-staging`; origen canónico activo `https://staging.espanolhonesto.com`; URL directa transitoria `https://espanolhonesto-staging.alindev95.workers.dev` | Worker objetivo `espanolhonesto`, todavía ausente | Staging tiene DNS, TLS y routing verificados; Preview URLs web desactivadas y `workers.dev` conservado para rollback/webhook test. El siguiente paso RC es crear el Worker production en `production_bootstrap`, sin rutas, DNS, tráfico ni secretos de proveedores. |
...
| Fulfillment | Worker `espanol-honesto-fulfillment-staging`, conectado al web por `FULFILLMENT_SERVICE` | Worker `espanol-honesto-fulfillment-production` | Staging tiene cron/secrets y pasó el smoke integral. Production existe en bootstrap inerte: versión `bf10884a-a4ce-4f34-95dc-e931aeb6e180`, cero secretos, cero Cron, proveedores ausentes y operación bloqueada con 503. Queue/DLQ production existen y están activas pero sin productores/consumidores. Falta cargar el HMAC mínimo, crear web bootstrap y atestiguar el mismo HMAC en ambos Workers. |
| Supabase | Proyecto `mzjyvmlxfpzdfdjzxxyj`, 48 migraciones hasta `20260713161300` | Proyecto `vkkahxsybhbutszerawz`, 24 entradas históricas | Proyectos separados y sanos. Staging está completo. Production conserva exactamente 25 migraciones RC pendientes, 138 usuarios/perfiles fixture y 27 suscripciones Stripe heredadas sin `package_price_id`; requiere Auth inerte, backup EFS fresco, política de preservación/limpieza y rollout por olas. Las dos migraciones staging-only están expresamente excluidas de production. |
...
El inventario vigente del 2026-07-13 encuentra 48 migraciones locales. Staging registra y verifica las cuatro migraciones RC `20260712112000`-`20260712115000`, `20260712195500_harden_sessions_status_contract.sql` y la migración exclusiva de hostname staging `20260713161300_allow_staging_custom_hostname.sql`; no queda una cola local pendiente para staging. Production mantiene 25 migraciones semanticas pendientes; el preflight las clasifica en siete olas, sin ambiguedades ni migraciones sin plan. `20260710150000_staging_integration_smoke_runs.sql` y su compatibilidad de hostname siguen siendo exclusivamente staging y nunca se aplican a production.

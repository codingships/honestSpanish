...
4. Ejecutar `pnpm launch:staging-smoke-rehearsal-runner` en modo plan y abrir `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/summary.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/staging-smoke-command-manifest.json`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/staging-smoke-execution-plan.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/approval-gate.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/cloudflare-checkout-gate-approval.md` y `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/rollback-after-staging-smoke.md`. En una Sandbox nueva, revisar tambien la frase exacta `STAGING_CHECKOUT_BOOTSTRAP_APPROVAL` y ejecutar primero `--bootstrap-checkout-approved`: prepara una sesion bajo Test Clock, copia la URL solo al portapapeles y cierra/verifica el gate antes de que se complete el pago. En modo smoke aprobado, el runner lanza primero `real-env-smoke.ts --preflight-only`; si cualquier precondicion falla, `externalWriteCommandStarted` sigue `false`.
...
7. Checkout real en Stripe test tras aprobacion CRM y las cuatro aceptaciones. El modo default del smoke estrecho solo abre/verifica/caduca una sesion y no demuestra pago; el modo bootstrap aprobado preserva exactamente una sesion verificada, pero tampoco demuestra pago hasta completar la URL copiada con el gate ya restaurado a `false`.

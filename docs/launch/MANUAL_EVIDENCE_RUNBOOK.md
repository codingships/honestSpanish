...
- Para el estado actual, usar la última salida de `pnpm launch:status`: Fase 1, staging y `production_inert_preparation` están cerrados. El RC técnico debe aparecer como `RC_READY_WITH_FINAL_BLOCKERS`; solo permanecen `legal_owner_controller`, `legal_human_review`, `integration_readiness`, `seo_llm_final` y `final_smoke`. `docs/launch/RC_EVIDENCE_REFRESH.md` es histórico, no una lista de trabajo viva.
...
4. El P3 `ERR-QA-SUPABASE-PROCESSED-AT-DEFAULT-149` quedó cerrado el 2026-07-17: staging `mzjyvmlxfpzdfdjzxxyj` y production `vkkahxsybhbutszerawz` registran `20260703211451` y verifican `processed_at_default=<NULL>`. Production lo aplicó como primera ola del rollout source-bound de 25 migraciones; evidencia: `outputs/launch-supabase-production-rollout-runner/2026-07-17T10-20-29-557Z/production-rollout-receipt.json`. Para comprobarlo de nuevo usar solo preflight/readback; no repetir la ola. El runner legacy `pnpm launch:supabase-processed-at-cleanup-runner` permanece retirado y fail-closed: `--execute-approved` debe fallar antes de red o SQL.
...
3. Renovar `pnpm launch:cloudflare-production-runtime-readonly` solo inmediatamente antes de una escritura final aprobada y abrir `outputs/launch-cloudflare-production-runtime-readonly/<timestamp>/summary.md`: confirmar cuenta, Workers production, versiones, bindings y secret names sin valores. Este comando solo lista/lee; parar si una herramienta intenta crear, desplegar, borrar, mover dominios o escribir secrets.
4. Tratar C-D-E como historia cerrada del 2026-07-17. Revisar, sin volver a ejecutar sus planes ni writes, `outputs/launch-cloudflare-production-fulfillment-bootstrap-secrets`, `outputs/launch-cloudflare-production-worker-phase1`, `outputs/launch-cloudflare-production-worker-bootstrap-secrets` y la atestación GET-only asociada. La expiración de una lectura no autoriza repetir bootstrap o HMAC.
5. Congelar primero `LAUNCH_SHA`: identidad legal real, SEO final, `main` limpio, remoto idéntico y CI verde. El `RC_BASE_SHA` técnico no autoriza un deploy activo.
6. Revisar `pnpm launch:cloudflare-production-runtime-cutover` como paquete local de secuencia/rollback, pero no interpretar sus aprobaciones históricas de bootstrap como una autorización vigente. La ventana final necesita gates ligados a `LAUNCH_SHA` para secrets activos, deploy web activo, enable fulfillment, dominios y checkout; además, el runner de secrets fulfillment debe incorporar lock/checkpoint write-ahead y reconciliación GET antes de poder usarse.
7. Ejecutar `pnpm launch:cloudflare-production-worker-secrets` únicamente en modo plan y solo cuando exista `LAUNCH_SHA`; revisar `summary.md`, `cloudflare-worker-secrets-command-manifest.json`, `cloudflare-worker-secrets-execution-plan.md`, `approval-gate.md` y `rollback-after-worker-secrets.md`. Confirmar checkout `false`, nombres sin valores y ausencia de dominios/DNS/pagos. No ejecutar mientras falten las fronteras de Auth production, acceso admin/profesor, catálogo Stripe Live, tráfico temporal, domain cutover y checkout final enumeradas en `FINAL_CLOSURE.md`.
8. Usar únicamente la secuencia exacta de `FINAL_CLOSURE.md`: secrets fulfillment endurecidos aún bootstrap → web activo con checkout cerrado → enable fulfillment → dominios bajo frontera de tráfico con signup/checkout cerrados → acceso admin/profesor → catálogo Live → revisiones legal/SEO/readiness → Go/No-Go → signup → checkout solo para operador → una compra propia → retirar frontera de tráfico. Si falta un runner, una aprobación exacta o un rollback verificable, `integration_readiness` permanece pendiente.
...
16. Conservar como historia cerrada la neutralización de `espanol-honesto-reminders` del 2026-07-14: Cron vacío, `workers.dev=false`, Preview URLs desactivadas y sin dominios/rutas. Solo repetir un GET si aparece deriva demostrada; no volver a modificarlo por rutina.
...
- `command_output`: `../../outputs/launch-cloudflare-production-fulfillment-bootstrap-secrets/<timestamp>/summary.md`, `approval-gate.md` y `command-manifest.json` como evidencia histórica HMAC-only de fulfillment ya ejecutada; no como instrucción para repetirla.
- `command_output`: `../../outputs/launch-cloudflare-production-worker-phase1/<timestamp>/summary.md`, `../../outputs/launch-cloudflare-production-worker-phase1/<timestamp>/phase1-command-manifest.json`, `../../outputs/launch-cloudflare-production-worker-phase1/<timestamp>/approval-gate.md` y `../../outputs/launch-cloudflare-production-worker-phase1/<timestamp>/rollback-after-phase1.md` como evidencia histórica de Worker phase 1 ya ejecutada; no como trabajo pendiente.
...
3. Reutilizar como historia cerrada el ciclo billing, el smoke integral staging y el simulacro de rollback del 2026-07-12/14. No ejecutar sus runners ni volver a mutar staging salvo que un cambio material de código/configuración demuestre deriva y exista otra autorización exacta.
4. Revisar sus summaries existentes: checkout test completado, billing terminal/reconciliado, seis emails allowlisted entregados, Drive/Docs/Calendar/Meet, reserva/conflicto, recordatorio, cancelación/completion/no-show, Admin Jobs y cleanup. Su antigüedad por sí sola no obliga a repetirlos.
5. Confirmar que `LAUNCH_SHA`, legal, `integration_readiness` y `seo_llm_final` están cerrados; que signup/checkout siguen cerrados; y que existe una frontera temporal de tráfico para que la prueba propia no acepte alumnos antes de tiempo.
6. En production no ejecutar `real-env-smoke.ts`. Completar `production-minimal-smoke-checklist.md` manualmente sobre `LAUNCH_SHA`: páginas públicas/legales, login admin/profesor recuperado deliberadamente, salud de proveedores, Auth redirects, catálogo Live, signup tras Go/No-Go y, solo con aprobación Stripe Live separada, una compra propia como máximo.
7. Abrir checkout únicamente para el operador autorizado, ejecutar esa compra y verificar webhook, Supabase, confirmación contractual, fulfillment, email, Drive/Calendar y Portal. Retirar la frontera de tráfico solo después del éxito; ante fallo, cerrar checkout y seguir el rollback de `FINAL_CLOSURE.md`.
...
- `command_output`: `../../outputs/launch-final-smoke-execution-pack/<timestamp>/approval-request-staging-smoke.md` y `staging-preflight-checklist.md` únicamente como historia del rehearsal ya cerrado, no como instrucción para repetirlo.
- `command_output`: `../../outputs/launch-staging-billing-lifecycle/<timestamp>/summary.json` como evidencia histórica canónica del ciclo billing gated, terminal y revalidado.
- `command_output`: `../../outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/summary.md`, `staging-smoke-command-manifest.json`, `approval-gate.md` y `rollback-after-staging-smoke.md` como evidencia histórica del smoke staging cerrado.
- `command_output`: `../../outputs/real-env-smoke/<timestamp>/summary.md` como evidencia histórica; no ejecutar el harness contra production.

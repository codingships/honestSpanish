...
4. Para el P3 `ERR-QA-SUPABASE-PROCESSED-AT-DEFAULT-149`, ejecutar `pnpm launch:supabase-processed-at-readonly-preflight` y confirmar el estado real de ambos proyectos. Staging `mzjyvmlxfpzdfdjzxxyj` ya contiene `20260703211451` y debe mostrar `processed_at_default=<NULL>`; production `vkkahxsybhbutszerawz` sigue abierta mientras muestre `now()`. Generar un preflight production fresco con `pnpm launch:supabase-production-readonly-preflight` y preparar la ola actual con `pnpm launch:supabase-production-rollout -- --through processed_at_small_fix --preflight outputs/launch-supabase-production-readonly-preflight/<timestamp>/summary.json`. El modo plan debe devolver `PLAN_ONLY_READY` y `externalWritePerformed=false`. El runner legacy `pnpm launch:supabase-processed-at-cleanup-runner` queda retirado y fail-closed: `--execute-approved` debe fallar antes de red o SQL. La escritura correcta es la ola source-bound de production, sola o como primera de las 25 migraciones en siete olas, siempre con su aprobacion exacta fresca.
...
- `command_output`: `../../outputs/launch-supabase-production-rollout-runner/<timestamp>/summary.json` y `wave-processed_at_small_fix-verify-readonly.sql` como plan/receipt source-bound de production.
- `command_output`: `../../outputs/supabase-processed-at-readonly-preflight/<timestamp>/summary.md` despues de ejecutar la ola, con defaults `NULL` en staging/production y agregados webhook limpios.
...
4. Ejecutar `pnpm launch:cloudflare-production-runtime-cutover-preflight` y abrir `outputs/launch-cloudflare-production-runtime-cutover-preflight/<timestamp>/summary.md` y `cloudflare-production-worker-variable-matrix.md`: confirmar build local `production_bootstrap`, `wrangler deploy --config dist/server/wrangler.json --dry-run`, `WEB_RUNTIME_MODE=bootstrap`, checkout/email desactivados, sin adjuntar dominios, limpieza de `dist` y matriz de nombres sin valores. El build activo `build:production:release` queda separado para la ventana final con datos legales y Stripe Live.
5. Ejecutar `pnpm launch:cloudflare-production-runtime-cutover` y abrir `outputs/launch-cloudflare-production-runtime-cutover/<timestamp>/cloudflare-production-runtime-cutover-manifest.json`, `approval-request-fulfillment-bootstrap-hmac.md`, `approval-request-phase-1-worker.md`, `approval-request-worker-secrets.md`, `approval-request-domain-move.md`, `verification-checklist.md` y `rollback-plan.md`.
6. Ejecutar `pnpm launch:cloudflare-production-fulfillment-bootstrap-secrets` en modo plan y abrir `outputs/launch-cloudflare-production-fulfillment-bootstrap-secrets/<timestamp>/summary.md`, `approval-gate.md`, `execution-plan.md` y `command-manifest.json`: confirmar `externalWritePerformed=false`, que el único nombre permitido es `INTERNAL_JOB_SECRET` y que Supabase/Google/Resend/cron quedan explícitamente aplazados.
7. Ejecutar `pnpm launch:cloudflare-production-worker-phase1` en modo plan y abrir `outputs/launch-cloudflare-production-worker-phase1/<timestamp>/summary.md`, `phase1-command-manifest.json`, `phase1-execution-plan.md`, `approval-gate.md` y `rollback-after-phase1.md`: confirmar que el deploy exige evidencia HMAC-only de fulfillment, `externalWritePerformed=false`, `PLAN_ONLY_READY`, `CLOUDFLARE_PHASE1_APPROVAL`, `--execute-approved`, `CHECKOUT_ENABLED=false`, sin mover dominios, sin DNS, sin Pages delete, sin secrets y sin pagos reales.
8. Ejecutar `pnpm launch:cloudflare-production-worker-secrets` en modo plan y abrir `outputs/launch-cloudflare-production-worker-secrets/<timestamp>/summary.md`, `cloudflare-worker-secrets-command-manifest.json`, `cloudflare-worker-secrets-execution-plan.md`, `approval-gate.md` y `rollback-after-worker-secrets.md`: confirmar `externalWritePerformed=false`, `PLAN_ONLY_READY`, `CLOUDFLARE_WORKER_SECRETS_APPROVAL`, `--execute-approved`, `CLOUDFLARE_WORKER_DIRECT_URL` opcional, nombres sin valores, sin mover dominios, sin DNS, sin Pages delete y sin pagos reales. Este runner completo queda para la ventana final; el bootstrap web usa su runner HMAC-only separado.
9. Revisar `outputs/019f1a5e-2745-7c43-870d-544e6ba4e0b1/strict-qa-v2/cloudflare-domain-worker-preflight.md`: confirmar Cloudflare Pages-vs-Worker/domain ownership, production Worker `espanolhonesto` si se usa Workers, production Worker secret-name posture, probes no destructivos de URL directa y mover `espanolhonesto.com`/`www.espanolhonesto.com` solo con aprobacion separada.
10. Revisar Google Drive root folder, template doc y admin account.
11. Revisar Google Calendar/Meet.
12. Revisar Resend sender/domain.
13. Revisar Sentry production unresolved issues.
   Ejecutar `pnpm launch:sentry-readonly -- --env-file .env --environment production --time-range 14d --limit 50` como evidencia agregada. Si faltan scrub IP o alertas, ejecutar `pnpm launch:sentry-production-hardening` en modo plan y revisar `outputs/launch-sentry-production-hardening/<timestamp>/approval-gate.md`; solo el modo literalmente aprobado puede crear los dos workflows production y habilitar scrub IP. Si avisa por issues no resueltos, ejecutar `pnpm launch:sentry-triage-pack` y `pnpm launch:sentry-issue-triage-runner` en modo plan; revisar `outputs/launch-sentry-triage-pack/<timestamp>/approval-request.md`, `triage-checklist.md`, `alert-ownership-checklist.md`, `manual-evidence-dry-run-accepted-risk.txt`, `manual-evidence-dry-run-pass.txt`, `outputs/launch-sentry-issue-triage-runner/<timestamp>/approval-gate.md` y `rollback-after-sentry-issue-triage.md` antes de cualquier triage dashboard, API status change o aceptacion de riesgo.
14. Revisar Turnstile domains.
...
15. Revisar fulfillment/reminder worker, `CRON_SECRET`, `INTERNAL_JOB_SECRET`, `FULFILLMENT_WORKER_URL` y `PUBLIC_SITE_URL`.
16. Confirmar que no queda ningun Cloudflare Worker legacy con cron activo que interfiera con `workers/fulfillment`, incluyendo `espanol-honesto-reminders`.
17. En Cloudflare Turnstile, confirmar que el widget asociado a `PUBLIC_TURNSTILE_SITE_KEY` cubre `espanolhonesto.com`, `www.espanolhonesto.com`, staging y Pages preview aplicable. Si se usa API, registrar solo site key parcial, dominios y modo; nunca secret key.
18. Si el conector Stripe no permite listar productos/precios, usar dashboard Stripe y evidencia de checkout/webhook/reconciliacion en vez del MCP.
...
- `command_output`: `../../outputs/launch-cloudflare-production-fulfillment-bootstrap-secrets/<timestamp>/summary.md`, `approval-gate.md` y `command-manifest.json` como runner HMAC-only de fulfillment; en modo plan debe mostrar `externalWritePerformed=false`, `INTERNAL_JOB_SECRET` como único nombre permitido y providers aplazados.
...
2. Ejecutar `pnpm launch:final-smoke-execution-pack` y abrir `outputs/launch-final-smoke-execution-pack/<timestamp>/final-smoke-execution-manifest.json`, `approval-request-final-smoke.md`, `production-minimal-smoke-checklist.md`, `approval-request-staging-smoke.md`, `preflight-checklist.md`, `staging-preflight-checklist.md` y `rollback-and-cleanup-plan.md`. El ciclo integral automatizado es exclusivo de staging; production usa el checklist minimo/manual.
3. Confirmar read-only que `CHECKOUT_ENABLED_OVERRIDE=false` en `espanolhonesto-staging` y que el probe devuelve 403. El runner no cambia Cloudflare. El bootstrap estrecho queda fuera del flujo activo; como excepcion manual exige un propietario externo que abra y restaure/verifique el gate porque `scripts/smoke-checkout.ts` tampoco escribe Cloudflare.
4. Ejecutar `pnpm launch:staging-smoke-rehearsal-runner` en modo plan y abrir `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/summary.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/staging-smoke-command-manifest.json`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/staging-smoke-execution-plan.md`, `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/approval-gate.md` y `outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/rollback-after-staging-smoke.md`. El smoke reutiliza la Checkout completada ya revisada y no crea ni expira otra. En modo aprobado, el runner lanza primero `real-env-smoke.ts --preflight-only --expect-checkout-override false`; si cualquier precondicion falla, `externalWriteCommandStarted` sigue `false`. El child con writes no tiene timeout duro impuesto por el runner: hay que dejarlo terminar su `finally`; una interrupcion forzada invalida la evidencia de cleanup y exige conciliacion manual.
...
7. Reutilizar la Checkout real Stripe test ya completada tras las cuatro aceptaciones; el smoke integral no abre, crea ni caduca otra sesion.
8. Ejecutar y registrar el ciclo billing canonico antes del harness integral:
   - Fijar `STAGING_BILLING_CHECKOUT_SESSION_ID=cs_test_...` o pasar `--checkout-session cs_test_...`; el `--timeout-ms` opcional acepta 30000-300000 ms para cada espera acotada.
   - Ejecutar `pnpm launch:staging-billing-lifecycle:preflight`. Es read-only y no requiere confirmacion de mutacion.
   - Si pasa, fijar exactamente `STAGING_BILLING_LIFECYCLE_CONFIRMATION=I_CONFIRM_STAGING_BILLING_LIFECYCLE:<same-session>` y ejecutar `pnpm launch:staging-billing-lifecycle`.
   - Revisar `outputs/launch-staging-billing-lifecycle/<timestamp>/summary.json`: `status=OK`, cuenta/proyecto/test y sesion exactas, `mutationGate.authorized=true`, `checkpoint.phase=complete`, sin error, recursos/eventos terminales y revalidacion final.
   - Si se interrumpe despues de crear `outputs/launch-staging-billing-lifecycle/checkpoints/<session>.json`, no editar ni borrar ese checkpoint; con la misma sesion/confirmacion ejecutar `pnpm launch:staging-billing-lifecycle:resume`. No usar `:resume` sin checkpoint: falla cerrado.
   - Fijar `SMOKE_COMPLETED_CHECKOUT_SESSION_ID=<same-session>` y `SMOKE_BILLING_LIFECYCLE_EVIDENCE_PATH=<ruta-real-absoluta-o-relativa-al-summary.json>`. El subprocess `--preflight-only` valida el artefacto y revalida Stripe/Supabase/webhooks vivos antes de cualquier write del harness; no existe una frase manual alternativa que pueda sustituirlo. No fabricar ni firmar eventos sinteticos.
...
17. Confirmar cleanup: suscripcion/sesiones temporales de scheduling, Docs/eventos y job/audits eliminados; notas, enlace Google y asignaciones restaurados; usuario/folder reutilizables y evidencia Checkout/CRM/pagos conservados. Confirmar que el gate siguio en `false` y responde `403 Checkout is disabled`; el runner no hace ningun write Cloudflare.
...
- `command_output`: `../../outputs/launch-final-smoke-execution-pack/<timestamp>/approval-request-staging-smoke.md` y `staging-preflight-checklist.md` como aprobacion y preflight del staging rehearsal cerrado.
- `command_output`: `../../outputs/launch-staging-billing-lifecycle/<timestamp>/summary.json` como evidencia canonica del ciclo billing gated, terminal y revalidado.
- `command_output`: `../../outputs/launch-staging-smoke-rehearsal-runner/<timestamp>/summary.md`, `staging-smoke-command-manifest.json`, `approval-gate.md` y `rollback-after-staging-smoke.md` como runner gated; debe mostrar el preflight read-only, el 403 y la reutilizacion de evidencia antes de cualquier write.

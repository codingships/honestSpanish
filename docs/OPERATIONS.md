...
Antes de un candidato de lanzamiento se despacha manualmente `Performance audit`: siete plantillas representativas, tres pasadas reproducibles y perfiles móvil y escritorio. El target `candidate` compila el SHA del workflow y mide el Worker de Cloudflare generado, servido localmente con bindings ficticios y proveedores externos desactivados; no incluye la compilación bajo demanda de `astro dev` ni pretende simular la red edge. Chrome/Lighthouse devuelve de forma reproducible un falso `NO_FCP` para el artículo prerenderizado servido por Wrangler local, aunque un navegador limpio observa HTML, contenido y FCP. Por eso solo ese recorrido usa en `candidate` una prueba explícita de pintura y consola; `staging` no admite esa excepción y debe producir Lighthouse completo para las siete rutas. `staging` solo se usa después de que el despliegue haya acreditado ese mismo SHA y exige introducirlo completo. Cada ejecución conserva JSON, HTML o el diagnóstico de pintura, el tipo de runtime y un resumen con mediana y peor resultado durante 30 días.

Los mínimos del workflow son suelos de regresión iniciales, no objetivos de negocio. El smoke conserva provisionalmente rendimiento 65 y LCP 7 s hasta disponer de una línea base del Worker compilado; los valores anteriores procedían del servidor de desarrollo y no describen el rendimiento de producción. La auditoría completa mantiene rendimiento 70 y LCP 4 s para hacer visible la deuda antes del lanzamiento. Los informes identifican el elemento LCP y separan TTFB, espera de recurso, descarga y demora de renderizado para que una optimización responda a una causa medida. El objetivo de campo, cuando haya tráfico suficiente, sigue siendo LCP ≤ 2,5 s, INP ≤ 200 ms y CLS ≤ 0,1 al p75. Staging y test son deliberadamente `noindex`; `is-crawlable` no bloquea allí y la indexabilidad de producción se verifica por separado en el gate live.

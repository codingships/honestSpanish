...
| A02 | Permisos administrativos granulares | IMPLEMENTADO | [`admin-access.ts`](../src/lib/admin-access.ts), [mapa central](../src/lib/admin-route-capabilities.ts), [migración](../supabase/migrations/20260803151112_admin_access_foundation.sql), [gestor](../src/components/admin/AdminAccessManager.tsx), [contrato SQL](../tests/sql/admin-access-foundation.sql) | Acreditar PR y staging. El gestor asigna permisos a perfiles ya administradores; la invitación/promoción segura de nuevo personal sigue pendiente. |

...
describe('legal terms version on automatic renewal', () => {
    ...
    it('states the exact initial charge, renewal anchor and guarantee in every language', () => {
        ...
        expect(terms).toContain('devolución exacta del valor contractual de todas las clases no consumidas');
        expect(terms).toContain('cuenta como clase consumida y reduce proporcionalmente la devolución disponible');
        ...
        expect(terms).toContain('exact refund for the contractual value of every unconsumed class');
        expect(terms).toContain('counts as consumed and proportionally reduces the available refund');
        ...
        expect(terms).toContain('точный возврат договорной стоимости всех неиспользованных занятий');
    });
});

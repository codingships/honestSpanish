...
describe('operations runbook launch readiness', () => {
    it('keeps CI runtime aligned with package engines and pnpm policy', () => {
        ...
        expect(deploy).toContain('Require successful CI for the exact commit');
        expect(deploy).toContain('Verify staging checkout is disabled');
        expect(deploy).toContain('CLOUDFLARE_ENV: "staging"');
        expect(deploy).toContain('deploy-built-worker.ts --environment staging --dry-run');
        expect(deploy).toContain('pnpm run deploy');
        expect(deploy).toContain('This privileged workflow must be dispatched from refs/heads/main.');
        expect(deploy).toContain('scripts/ci/verify-staging-deploy-environment.ts');
        expect(deploy).toContain('pnpm run launch:staging-operations -- --include-wrangler');
        expect(deploy).not.toContain('pnpm run deploy -- --dry-run');
        expect(deploy).toContain('pnpm run build:staging:release');
        expect(deploy).toContain('pnpm exec wrangler deploy --config workers/fulfillment/wrangler.toml --env staging --keep-vars');
        expect(deploy).not.toContain('--env production');
        expect(deploy).not.toContain('--environment production');
        expect(deploy).not.toContain('run deploy -- --env');
        expect(deploy).toContain('PUBLIC_SITE_URL: "https://staging.espanolhonesto.com"');
        expect(deploy).toContain('FULFILLMENT_WORKER_URL: "https://espanol-honesto-fulfillment-staging.alindev95.workers.dev"');
        const buildIndex = deploy.indexOf('name: Build exact staging package');
        const firstDryRunIndex = deploy.indexOf('name: Validate staging Fulfillment package');
        expect(buildIndex).toBeGreaterThan(-1);
        expect(firstDryRunIndex).toBeGreaterThan(buildIndex);
        expect(deploy).toContain('STAGING_WORKER_URL: https://staging.espanolhonesto.com');
        expect(deploy).not.toContain('vars.CLOUDFLARE_STAGING_URL');
        expect(deploy).not.toContain('vars.CLOUDFLARE_WORKERS_STAGING_URL');
        expect(deploy).toContain('--deployed-url "$STAGING_WORKER_URL"');
        ...
        expect(audit).toContain("path.join('.github', 'workflows', 'deploy-staging.yml')");
    });
    ...
    it('keeps Cloudflare production domain cutover separate from Worker creation and secrets', () => {
        ...
        for (const snippet of [
            ...
            'CHECKOUT_ENABLED=false',
            'currentActiveBuildSource',
            'currentWebTransitionRunner',
            'currentWebTransitionFaultTests',
            'commandizedWranglerProductionDryRun',
            'cloudflareRuntimeCutoverPreflight',
            'cloudflare-production-worker-variable-matrix.md',
            'launch:cloudflare-production-runtime-cutover-preflight',
            'cloudflare_runtime_cutover_preflight_exists',
            'Current source and executable tests define a clean, validated active build',
            'local_build_parity_evidence_exists',
            'cloudflare-production-web-active-orchestrator.test.ts',

...
describe('operations runbook launch readiness', () => {
    it('keeps CI runtime aligned with package engines and pnpm policy', () => {
        ...
        expect(ci).toContain('name: Resolve exact runtime URLs');
        expect(ci).toContain('PRODUCTION_PUBLIC_SITE_URL: ${{ vars.PUBLIC_SITE_URL }}');
        ...
        expect(ci).not.toContain("github.ref_name == 'main' && secrets.FULFILLMENT_WORKER_URL ||");
        const resolveRuntimeUrlsIndex = ci.indexOf('name: Resolve exact runtime URLs');
        const deployBuildIndex = ci.indexOf('      - name: Build\n', resolveRuntimeUrlsIndex);

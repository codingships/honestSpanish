...
function renderPackage(report: IntegrationPackageReport): string {
    ...
    lines.push(
        ...
        '- Reconfirm that preserved legacy Worker `espanol-honesto-reminders` remains fully neutralized and cannot be recreated from a tracked deployment path.',
...
function renderApprovalChecklist(report: IntegrationPackageReport): string {
    return `${[
        ...
        '- Legacy Worker neutralization reconfirmed from fresh read-only evidence.',
...
function renderManifest(report: IntegrationPackageReport, renderedFiles: Omit<RenderedArtifacts, 'manifest'>): string {
    return `${JSON.stringify({
        ...
        finalReviewRequirements: [
            ...
            'fulfillment/reminder worker and legacy Worker neutralization posture',
...
function remediationPlanForSource(source: EvidenceSource): WarningRemediation[] {
    ...
    if (source.id === 'turnstile_readonly') {
        return [{
            ...base,
            allowedNextStep: 'Run pnpm --config.verify-deps-before-run=false launch:turnstile-domain-closure-pack and pnpm --config.verify-deps-before-run=false launch:turnstile-domain-closure-runner. Then use the encrypted Wrangler OAuth keyring evidence or exact-approved runner/dashboard evidence confirming the configured Turnstile site key belongs to the intended widget and covers espanolhonesto.com, www.espanolhonesto.com and staging.espanolhonesto.com.',
            readOnlyVerification: 'Rerun pnpm --config.verify-deps-before-run=false launch:turnstile-readonly and confirm cloudflare_wrangler_oauth_readonly and turnstile_widgets_readonly are OK; no Cloudflare API token or clipboard input is accepted.',
    ...
    if (source.id === 'supabase_processed_at_cleanup_runner') {
        return [{
            ...base,
            allowedNextStep: 'Generate a fresh production read-only preflight, then run pnpm launch:supabase-production-rollout -- --through processed_at_small_fix --preflight <fresh-summary.json> in plan mode. Execute that source-bound wave only under its generated exact approval, or include it as the first wave of the exact 25-migration rollout.',

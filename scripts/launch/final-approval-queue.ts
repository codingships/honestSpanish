    ...
    return [
        ...
        },
        {
            ...
            forbiddenScope: [
                ...
                'No domain move',
                'No Stripe live mode or checkout enablement',
            ],
        ...
        },
        {
            ...
            prerequisiteItemIds: ['cloudflare_worker_create', 'cloudflare_worker_secrets'],
            status: 'missing_artifacts',
        },
        {
        ...
        },
        {
            ...
            allowedScope: 'Run staging rehearsal before legal/live-domain final closure to find technical, logistical and UX issues using Stripe test and approved smoke credentials; this does not close final_smoke.',
            forbiddenScope: [
                ...
                'No Stripe live mode, real charge or CHECKOUT_ENABLED=true',
            ],
        ...
        },
        {
            ...
            title: 'Final write-capable lifecycle smoke',
            kind: 'external_write_approval',
            target: 'Final SMOKE_BASE_URL host only, guarded by SMOKE_EXTERNAL_WRITES_CONFIRMATION=writes-ok:<host>',
            ...
            allowedScope: 'Run the final smoke only after runtime/domain/payment/legal/provider posture is final enough and the exact writes-ok host approval is present.',
            forbiddenScope: [
                'No production smoke without exact host confirmation',
                'No secret/private payload evidence',
...
function buildCriticalPath(queueItems: QueueItem[]): CriticalPathStep[] {
    ...
    return [
        ...
        },
        {
            ...
            title: 'Load secret names and probe direct Worker URL',
            phase: 'Cloudflare phases 2-3',
            itemIds: [requireItem('cloudflare_worker_secrets')],
            prerequisites: [
            ...
            closeWhen: 'Required secret/var names are present by name only and direct workers.dev probes pass the modern public surface.',
            stopIf: [
...
function renderNextActionCursor(report: QueueReport): string {
    ...
    const lines = [
        ...
        mustWaitLine(cloudflareDomain, 'Cloudflare production domain move', 'wait until Worker shell, secret-name verification and direct Worker URL probes have passed with separate domain approval.'),

...
function renderMigrationBundle(artifactToRender: MigrationArtifact): string {
    const lines = [
        ...
        '-- Current sequence: staging already applied/verified; production uses the source-bound processed_at_small_fix rollout wave.',
        '-- Staging evidence target: espanol-staging (mzjyvmlxfpzdfdjzxxyj).',
        '-- Production write target: espanol-honesto (vkkahxsybhbutszerawz).',
        '',
        '-- ============================================================================',
...
function renderPreflightSql(): string {
    return `${[
        ...
        '-- Read-only metadata/aggregate queries. Run against both environments; staging is evidence-only and production remains the write target.',
        '',
        'select version',
...
function renderApprovalRequest(report: CleanupReport): string {
    return `${[
        ...
        '',
        'This local file is not permission. It documents ERR-QA-SUPABASE-PROCESSED-AT-DEFAULT-149 and redirects execution to the source-bound production rollout.',
        '',
        '## Target Resources',
        '',
        '- Staging evidence only, already applied: Supabase project `espanol-staging` (`mzjyvmlxfpzdfdjzxxyj`, eu-central-1).',
        '- Remaining write target: Supabase project `espanol-honesto` (`vkkahxsybhbutszerawz`, eu-west-1).',
        '',
        '## Migration',
        '',
        ...
        '- Run `pnpm launch:supabase-processed-at-readonly-preflight` and confirm staging remains applied/clean.',
        '- Run `pnpm launch:supabase-production-readonly-preflight` and bind the fresh summary to the production rollout plan.',
        ...
        '',
        '## Current Approval Source',
        '',
        'Generate the hash-bound sentence with `pnpm launch:supabase-production-rollout -- --through processed_at_small_fix --preflight outputs/launch-supabase-production-readonly-preflight/<timestamp>/summary.json` and use only `exact-approval-required.txt` from that same run. The legacy staging-first approval is retired.',
        '',
        '## Stop Gates',
        '',
        '- Stop if staging verification does not show migration `20260703211451`, `processed_at` column_default null and clean webhook aggregate counts.',
...
function renderManualEvidenceDryRun(report: CleanupReport): string {
    ...
    return `${[
        ...
        '  --status pass',
        '  --summary "Supabase production processed_at_small_fix applied by the source-bound rollout; read-only verification confirms both defaults NULL and clean webhook aggregates."',
        '  --environment "Supabase production vkkahxsybhbutszerawz; staging mzjyvmlxfpzdfdjzxxyj evidence-only"',
        '  --owner Alin',
        ...
        '  --evidence "manual_note=Replace with concrete non-secret result: staging remained applied/clean; production rollout receipt verified; fresh dual-environment preflight passed; strict-QA tracker updated."',
        '',
...
function renderAcceptedRiskPackage(report: CleanupReport): string {
    ...
    return `${[
        ...
        '',
        '- Preferred mitigation remains applying the source-bound production `processed_at_small_fix` wave, then verifying both environments read-only.',
...
function renderManifest(
    ...
    return {
        ...
        closureOptions: [
            'preferred: apply migration 20260703211451 through the source-bound production processed_at_small_fix wave, then verify both environments read-only',
        ...
        allowedScope: [
            ...
            'Treat staging mzjyvmlxfpzdfdjzxxyj as already-applied read-only evidence only.',
            'Run the source-bound production wave against vkkahxsybhbutszerawz only after fresh staging/production preflight.',

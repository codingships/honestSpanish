            ...
            if (preflightValidation.valid && mode === 'preflight-readonly') {
                ...
                if (preflightValidation.historyState === 'none') {
                    ...
                    checks.push({
                        ...
                        message: 'Both exact history versions already exist, so the runner skipped the write and moved to verification.',
...
function renderExecutionPlan(report: RunnerReport): string {
    return `${[
        ...
        '## Stop conditions',
        '',
        '- Stop before connecting if either local SHA-256 changes.',
...
function renderRollbackPlan(report: RunnerReport): string {
    return `${[
        ...
        '',
        'There is intentionally no automatic rollback command. Both changes tighten invariants and do not transform existing rows; a forward fix is safer than silently weakening them.',
        '',
...
function renderSummary(report: RunnerReport): string {
    return `${[
        ...
        '',
        'Default plan mode is entirely local. No Supabase or other service is contacted unless a future operator explicitly chooses read-only preflight or supplies both execute approval gates.',
        '',
    ].join('\n')}\n`;
}

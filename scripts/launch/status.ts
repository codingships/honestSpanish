...
function strictQaBlockerClosureRow(blocker: Finding, paths: ClosureWorksheetPaths): string {
    ...
    const closeItWith =
        findingId === 'ERR-QA-SUPABASE-PROCESSED-AT-DEFAULT-149'
            ? `pnpm launch:supabase-processed-at-cleanup; pnpm launch:supabase-processed-at-cleanup-runner; ${paths.supabaseProcessedAtCleanupApproval}; ${paths.supabaseProcessedAtCleanupManifest}; ${paths.supabaseProcessedAtCleanupPreflight}; ${paths.supabaseProcessedAtCleanupPostApplyVerification}; ${paths.supabaseProcessedAtAcceptedRiskPackage}; ${paths.supabaseProcessedAtAcceptedRiskDryRun}; ${paths.supabaseProcessedAtReadonlyPreflight}; ${paths.supabaseProcessedAtCleanupRunner}; ${paths.supabaseProcessedAtCleanupRunnerApprovalGate}; outputs/019f1a5e-2745-7c43-870d-544e6ba4e0b1/strict-qa-v2/supabase-processed-at-default-approval-package.md; supabase/migrations/20260703211451_drop_processed_webhook_processed_at_default.sql`
            : fix;
    const evidenceMinimum =
        findingId === 'ERR-QA-SUPABASE-PROCESSED-AT-DEFAULT-149'
            ? 'Run pnpm launch:supabase-processed-at-cleanup and pnpm launch:supabase-processed-at-cleanup-runner. Then either use the gated runner with exact approval to apply/verify staging first and production second, or use the generated accepted-risk package/dry-run with explicit Alin acceptance. Update strict-qa-results.json, rebuild strict-qa-tracker-v2.xlsx and rerun pnpm launch:status.'

...
function validateBootstrapBundle(distRoot: string, sourceValues: Map<string, string>): void {
    ...
    for (const filePath of files) {
        ...
        for (const [key, value] of sourceValues) {
            if (contents.includes(value)) leakedNames.add(key);
        }
...
function validateGeneratedBootstrap(configPath: string): void {
    if (!existsSync(configPath)) {
        throw new Error('[build:production:bootstrap] Generated dist/server/wrangler.json is missing.');
    }
    ...
    const forbiddenNames = [
        ...
        'CRON_SECRET',
        'LEVEL_CHECK_TOKEN_SECRET',
    ].filter((key) => Object.prototype.hasOwnProperty.call(vars, key));
